...
The worker's state is under `paths.stateRoot`, by default `~/.local/state/pi-stack-publication`. Requests are `queued`, `running`, `failed` or `published`. A busy first-activation gate returns to `queued` with `blockedSince`, `waiting.reason` and `nextAttemptAt`. Cancellation is a failed request with reason `cancelled`. Requests and repair receipts are the issue source of truth. `deploy/publication issues` derives their complete index and maintains one bounded alert at the configured inbox; `acknowledge-issues` silences the current notification without clearing repair or host custody.
