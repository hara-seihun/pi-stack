...
A permanent native startup failure, including a missing model or rejected cwd, settles the assignment as failed and notifies its parent. Other startup failures get three attempts, with retry times and counts persisted per assignment in `startupFailure` metadata. Failed preparation counts too. The owner confirms runner absence or native cancellation before recording failure; an uncertain attachment or cancellation keeps the execution claim intact and exposes its error. The thread holds remaining input across restarts. Capacity admission waits do not consume the startup retry budget, and an already attached active execution is not failed for being slow.

Stop also handles a rejected in-flight open. It proceeds to workspace-independent attachment rather than returning the startup error as a cancellation failure. The workspace manager checks owner-local durable pending and active thread records before reclaiming released checkouts. An idle historical thread does not retain its checkout indefinitely; reusing it after reclamation produces the explicit startup failure above.
