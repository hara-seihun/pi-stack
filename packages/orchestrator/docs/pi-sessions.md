...
Tracking covers prompt preflight, native agent runs, queued inputs, custom-message turns, tools, shell commands, compaction and tree navigation. Each operation inherits its cancellation signal through async-local scope, so a callback from a cancelled run cannot start work after a later run is admitted. Plain prompts cannot overlap preflight or an unsettled turn. Streaming steering uses Pi's queue in `all` mode for thread sessions: every steer waiting at a turn boundary enters the next model request together, with its own message and work receipt. This is session-local and does not change interactive Pi's steering preference. A steer arriving after the boundary enters a later request.
