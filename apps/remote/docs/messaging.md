...
The browser keeps human-message drafts, drawings, attachment selections and preloaded history in memory only, and clears them when the person locks or changes. Supervisor JSON is compressed with Brotli or gzip when accepted. GET JSON responses use private ETag revalidation (link card metadata may remain fresh for five minutes); preview images use private immutable caching at content-addressed URLs. The router varies cached responses by person, session and cookie, and image URLs carry the person's session. Attachment downloads and mutating responses remain `no-store`; the messaging client does not explicitly write transcripts or credentials to IndexedDB or localStorage, though the browser may retain private HTTP cache entries. Signal sessions remain on the host. Downloading a file deliberately creates a device-local copy outside PiStack's encrypted host storage. The phone still holds its PiStack unlock credential according to the existing person-session policy.
