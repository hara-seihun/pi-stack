...
- `messaging/preview-images`: content-addressed, browser-safe link card images (SHA-256 filenames), retained across supervisor restarts;
...
The browser keeps human-message drafts, drawings, attachment selections and preloaded history in memory only, and clears them when the person locks or changes. Supervisor JSON is compressed with Brotli or gzip when accepted. GET JSON responses use private ETag revalidation (link card metadata may remain fresh for five minutes); preview images use private immutable caching at content-addressed URLs. The router varies cached responses by person, session and cookie, and image URLs carry the person's session. Sent and received attachments use private immutable caching with ETags because their IDs never change content. Draft, sending, failed and uncertain-send attachments remain `no-store`, as do mutating responses. The router permits fresh private caching only for an authenticated session-bearing URL; session-less requests revalidate through authorization before cache reuse; the messaging client does not explicitly write transcripts or credentials to IndexedDB or localStorage, though the browser may retain private HTTP cache entries. Signal sessions remain on the host. Downloading a file deliberately creates a device-local copy outside PiStack's encrypted host storage. The phone still holds its PiStack unlock credential according to the existing person-session policy.
...
Signal reaction-only receive events update the target message rather than creating an empty chat bubble or unread message. A reaction received before its target stays in `messages.sqlite3` and appears when that message arrives. Replayed events and older events do not undo newer add or remove events. Group reactions use the group conversation and the target author's address, so messages with the same timestamp from different group members do not share reactions. History includes `identity` and `reactions` on each message; reaction senders resolve through the same contact directory as message authors, and `own` marks the linked account's reaction.

The shared `POST /v1/messages/reactions` route accepts the universal `messaging/<local message id>` reference, emoji and optional remove flag. It calls `MessagingEndpoint.react(localMessageId, emoji, remove)`. The service checks that the target is a confirmed message in this account, then asks the backend to react using its original author and timestamp. Signal routes direct reactions to the contact and group reactions to the group. A successful backend receipt updates durable reaction state and advances the messaging snapshot version. Failed or uncertain backend sends do not invent a reaction. Plugins without `react` return `reactions_unsupported`; they can still receive messages normally.

### Replies

Incoming Signal `dataMessage.quote` and linked-device sent-message quotes keep the quoted author, timestamp and text in `messages.sqlite3`. History returns a `reply` with that preview and a universal `messaging/<local message id>` reference when the original is stored. If the original is absent, `messageId` is null; it resolves when the original arrives later, even after a restart. Resolution checks the conversation, canonical sender aliases and timestamp, so group members with matching timestamps cannot cross-link. Contact names follow directory changes without rewriting the quote.

A send may include `replyTo` with the universal reference of a confirmed message in the same conversation and backend. The server looks up the original and gives the plugin its stored author, timestamp and text. Signal sends `quoteTimestamp`, `quoteAuthor` and `quoteMessage` through its `send` JSON-RPC call, producing a real Signal reply. The durable send receipt stores the quote preview. The request ID also binds `replyTo`, so an uncertain send cannot be repeated with a different target. Missing, provisional, failed, uncertain, cross-conversation and non-messaging targets are rejected before dispatch. A reply to a message that is no longer in local history can still arrive from Signal with its quote preview, without a fabricated link.

### Link previews

The chat shows cards for up to three distinct HTTP(S) URLs in a stored message, including messages received before this feature existed. `GET /v1/messaging/messages/:messageId/link-previews` returns `{previews: MessagingLinkPreview[]}`; the router authenticates the person and the service reads only that person's encrypted message store. A missing message returns 404. The shared [`extractMessageLinks`](../server/messaging/links.ts) function keeps browser preview detection and selection aligned. Message text stays literal; the card itself is clickable. Previews are resolved on demand, not written into Signal history.

The service reads Open Graph, Twitter and page-title metadata from the first 512 KiB of HTML, including pages that declare a larger Content-Length. Each DNS answer is checked for private IPv4 destinations, and the request pins the checked address, including on redirects; IPv6-only targets get a plain hostname card. Metadata images are fetched with the same restrictions and returned only as capped inline raster data, never as a browser request to the publisher. The in-memory cache holds at most 256 links, expires successful cards after an hour and simple cards after five minutes. Four external preview jobs run at once; eight more wait for a slot. A full queue answers 429 rather than silently replacing queued cards, so the client can retry. A failed or slow site gets a plain hostname card without affecting the message.
...
PiStack stores messages received after linking. signal-cli does not provide an old-chat-history API or restore Android backups. Contact and group synchronization does not import past messages. Message edits, deletion, group administration and remote read receipts are not implemented. Disappearing and view-once messages are omitted with a visible backend status instead of being retained forever.
...
The supervisor sends the versioned messaging snapshot as its own `messaging` event on each client's stream whenever that version moves, carrying the conversations that are recent, unread or open; the endpoints below return the complete directory. The client preloads the newest 50 messages per Signal conversation, most recently active conversation first, with three parallel background requests and a fourth slot reserved for the selected chat. Selecting an uncached chat starts its request without waiting for background preloads; returning to a cached chat paints immediately. Preloading does not mark a conversation read. Older messages load on demand through ordinary cursor paging. The initial request never asks for an unbounded time window. Message groups and their segments mount newest-first while reverse-column layout preserves chronological reading order. Attachment images, audio/video metadata and link cards load independently near the viewport, with reserved-size local loading states and local retry controls. Media does not extend the page-level loading bar. Text and the composer are usable before media finishes. After that first window, the client fetches only differences. Every conversation in the snapshot carries a `revision`, and history responses carry the revision they reflect. A client requests a conversation again only when the snapshot announces a higher revision than it holds. It then asks `GET …/messages?after=REVISION&from=SEQUENCE`, where `from` is its window's `before` cursor, or 0 when it holds the whole history. The server answers `{messages, removed, revision}` with only the messages at or after `from` whose rendering changed after `after`, and the ids removed since then. Opening a chat, marking it read and a message arriving elsewhere therefore cost no history traffic for the other conversations. Returning to the app retries only failed loads; the stream's snapshot drives everything else.

SQLite triggers in `messages.sqlite3` own revisions, so no write path can forget to advance one. A message receives a new revision from one counter when it is inserted or updated, gains or loses an attachment or a reaction, when a quote target arrives, changes or disappears, and when the name, alias, avatar or account a message renders changes. Removing a message records its id and sequence in `message_removals`. Each revision also raises its conversation's revision.

`GET /v1/messaging/conversations/:conversationId/messages?since=TIMESTAMP` accepts a nonnegative millisecond timestamp without `before`. It returns every message at or after that timestamp, plus any intervening older-timestamp messages in sequence order, and at least the latest `limit` messages, even if none qualify by timestamp. This contiguous window matters when Signal delivers a message late with an older timestamp. Its `before` response cursor pages earlier sequence numbers with the ordinary history request. Without `since`, history keeps its usual page limit of 1 to 100 messages. `before` and `since` cannot be combined. The authenticated messaging API lists profiles and conversations, opens or closes current chats, pages message history, marks local unread state, uploads draft attachments, sends with request receipts, and serves owned attachment IDs. `POST /v1/messaging/backends/:backendId/link` starts a device link with an optional `{deviceName}` body, and `DELETE /v1/messaging/backends/:backendId/link` abandons one that is waiting; both answer with the current link state, which also travels in the messaging snapshot, so clients do not poll. Messaging attachments cannot be borrowed from agent threads, other conversations or another person's account. Inline previews admit raster image formats; other files download as attachments rather than execute as app-origin HTML or SVG.
