...
export function threadStateInstructions(options: {
  ...
  return [
    ...
    file,
    ...(options.activeTime === undefined ? [] : [
      `Current system time at this turn: ${new Date(options.activeTime).toISOString()}. Pi Remote identifies persisted user and assistant messages in your model context with a Message ID, sender, and the message's original time. Call message_react with that Message ID when reacting to a particular message. These labels are request-only context, not part of the stored message body.`,
    ]),
