...
An expired lease with a pending or running thread, live process, active systemd unit, Docker container or registered Git dependent remains referenced. Thread references come from the invoking Unix person's fleet database and application databases under the fleet ledger directory, plus that person's Remote database. The Remote path comes from `PI_REMOTE_DATA` or the invoking person's registered environment, and `PI_THREAD_DATABASE` adds the calling session's owning database. An existing database that cannot be read blocks cleanup. Finished idle threads do not keep historical checkouts indefinitely; queued input, including held input, and active execution claims do. Cleanup checks these durable references again before removing generated caches or the checkout. Work admitted concurrently after the last check still needs ThreadService's startup guard to fail visibly if its checkout is gone.

A linked worktree's common Git directory depends on its parent clone even when the worktree is leased and the parent is clean. The manager retains the parent until the child has been released. An unreadable linked-worktree Git directory stops cleanup rather than treating it as an absent dependency. `--reap-expired` fences runtime owners, stops user units, removes containers, terminates processes, and checks Git dependents again; it does not remove a parent still used by a linked worktree. System units remain blocked for their owning service lifecycle. Use reaping only for roots whose workers honor workspace leases.
