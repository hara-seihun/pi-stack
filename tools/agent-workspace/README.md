...
An expired lease with a pending or running thread, live process, active systemd unit, or Docker container remains referenced. Thread references come from the invoking Unix person's fleet database and application databases under the fleet ledger directory, plus that person's Remote database. The Remote path comes from `PI_REMOTE_DATA` or the invoking person's registered environment, and `PI_THREAD_DATABASE` adds the calling session's owning database. An existing database that cannot be read blocks cleanup. Finished idle threads do not keep historical checkouts indefinitely; queued input, including held input, and active execution claims do. Cleanup checks these durable references again before removing generated caches or the checkout. Work admitted concurrently after the last check still needs ThreadService's startup guard to fail visibly if its checkout is gone. `--reap-expired` fences that owner, stops user units, removes containers, terminates processes, and continues reconciliation. System units remain blocked for their owning service lifecycle. Use reaping only for roots whose workers honor workspace leases.
